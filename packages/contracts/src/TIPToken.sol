...
/**
 * @title TIP Token
 * @dev Core ERC-20 token for StoryHouse.vip ecosystem
 *
 * This is the base token contract that handles:
 ...
 */
contract TIPToken is ERC20, ERC20Burnable, Ownable, Pausable {
    // Events
    ...
    modifier onlyMinter() {
        require(minters[msg.sender], "TIPToken: caller is not a minter");
        _;
    }

    constructor(address initialOwner) ERC20("TIP Token", "TIP") Ownable(initialOwner) {
    ...
    function addMinter(address _minter) external onlyOwner {
        require(_minter != address(0), "TIPToken: zero address");
        require(!minters[_minter], "TIPToken: already a minter");
    ...
    function removeMinter(address _minter) external onlyOwner {
        require(minters[_minter], "TIPToken: not a minter");
    ...
    function mint(address to, uint256 amount) external onlyMinter whenNotPaused {
        require(totalSupply() + amount <= supplyCap, "TIPToken: supply cap exceeded");
        _mint(to, amount);
    }
    ...
    function updateSupplyCap(uint256 newCap) external onlyOwner {
        require(newCap >= totalSupply(), "TIPToken: cap below current supply");
